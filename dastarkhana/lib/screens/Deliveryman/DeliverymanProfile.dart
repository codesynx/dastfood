...
class _DeliverymanProfileState extends State<DeliverymanProfile> {
  ...
  Future<void> _loadUserData() async {
    ...
    try {
      ...
      _showSnackBar('Қолданушы деректерін жүктеу кезінде қате шықты: $e');
    } finally {
  ...
  Future<Map<String, dynamic>> _fetchUserById() async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Қолданушы деректерін алу мүмкін болмады: ${response.body}');
    }
  }
  ...
  Future<void> _updateUser() async {
    ...
    try {
      ...
      _showSnackBar('Профиль сәтті жаңартылды!');
    } catch (e) {
      _showSnackBar('Профильді жаңарту мүмкін болмады: $e');
    } finally {
      setState(() => _isLoading = false);
    }
  }
  ...
  Future<void> _updateDeliveryMan(int userId, Map<String, dynamic> updatedData) async {
    ...
    if (response.statusCode != 200) {
      throw Exception('Курьер туралы мәліметті жаңарту мүмкін болмады: ${response.body}');
    }
  }
          ...
          child: Column(
            children: [
              ...
              _buildTextField(
                ...
                validator: (value) {
                  if (value == null || value.isEmpty) {
                    return 'Телефон нөмірін енгізу керек';
                  }
                  return null;
                },
              ),
